import 'semantic-ui-css/semantic.min.css';
import React, { Component } from 'react';
import { Header } from '@bpanel/bpanel-ui';
import { Segment, Card, Feed, Label, Divider } from 'semantic-ui-react';
import UserList from './UserList';
import Form from './Form';
import SelectedUser from './SelectedUser';
import TransactionRow from './TransactionRow';

export default class BmoPlugin extends Component {
  render() {
    if (!this.props.bmo.currentUser) {
      return null;
    }
		console.log('IN THE COMPOENTN THIS IN THE PROPS!!');
		console.log(this.props.bmo);
    const { currentUser, selectedUser, transactions } = this.props.bmo;
    const { friends } = currentUser;
    return (
      <div>
        <Header type="h2">₿-Mo! - Venmo for Bitcoin</Header>
        {/* Tab Label */}
        <div className="ui top attached tabular menu">
          <div className="active item">
            <img
              style={{ width: '20px', height: '20px', marginRight: '10px' }}
              src={`https://robohash.org/${currentUser.account
                .email}.png?set=set4`}
            />
            <p>
              {currentUser.account.name ||
                currentUser.account.email ||
                "G'day!"}
            </p>
          </div>
        </div>
        {/* BEGIN - Tab Contents */}
        <div className="ui bottom attached active tab segment">
          <Segment
            className="container"
            style={{ backgroundColor: 'rgba(255,255,255,0.9)' }}
          >
            <div className="row">
              <div className="col-lg-4">
                {selectedUser ? (
                  <SelectedUser user={selectedUser} />
                ) : (
                  <span>Choose a friend :)</span>
                )}
              </div>
              <div className="col-lg-8">
                <Form
                  sendPayment={this.props.sendPayment}
                  currentUser={currentUser}
                  selectedUser={selectedUser}
                />
              </div>
            </div>
            <Divider fitted className="my-4" />
            <div className="row">
              <div className="col-lg-4">
                <UserList selectUser={this.props.selectUser} users={friends} />
              </div>
              <div className="col-lg-8">
                <Card className="w-100">
                  <Card.Content>
                    <Card.Header>Transaction History</Card.Header>
                  </Card.Content>
                  <Card.Content>
                    <Feed>
                      {transactions.length ? (
                        transactions.map(txn => {
                          return <TransactionRow transaction={txn} />;
                        })
                      ) : (
                        <h3
                          className="header"
                          style={{
                            textAlign: 'center',
                            color: 'rgba(0, 0, 0, 0.4)'
                          }}
                        >
                          No transactions yet...
                        </h3>
                      )}
                    </Feed>
                  </Card.Content>
                </Card>
              </div>
            </div>
          </Segment>
        </div>{' '}
        {/* END - Tab*/}
      </div>
    );
  }
}
